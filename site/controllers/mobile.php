...
class gglmsControllerMobile extends JControllerLegacy
{
    ...
    public function pulldata()
    {
        ...
        $mobileCoursesGroups = (array_map($func, $gruppiCorsi));

        $availableCourses = array_intersect($gruppiUtente, $mobileCoursesGroups);

        $availableCoursesList = implode(",", $availableCourses);

        $units = $this->getCourses($availableCoursesList);
    ...
    protected function getCourses($coursesList)
    {
        ...
        $query->join('inner', '#__gg_usergroup_map AS m ON u.id = m.idunita');
        $query->where("m.idgruppo in ($coursesList)");
    ...
    public function pushdata()
    {
        ...
        $scormvars = json_decode($inputJSON);

        foreach ($scormvars as $scormvar) {
            $this->storeScorm($scormvar);
        }

        echo json_encode(["sync" =>1]);

        $this->_japp->close();
    }


    public function storeScorm($data)
    {
        ...
        $query = $this->_db->getQuery(true)
            ->select('*')
            ->from('#__gg_scormvars')
            ->where("scoid =" . $data->scoid)
            ->where("userid =" . $data->userId)
            ->where("varName =" . $this->_db->quote($data->varName))
            ->where("varValue =" . $this->_db->quote($data->varValue));

        $this->_db->setQuery($query);
        $scormVar = $this->_db->loadAssoc();


        if ($scormVar || $scormVar->varValue != 'completed' || $scormVar->varValue != 'passed') {
